...
```
 ...
 GND ─┤8   9├─ Q7' (serial out, for daisy-chaining)
       └────┘
```
...
```
Instruction register field [3:0] → 4-to-16 decoder → enables one of 16 registers
Data bus [31:0] → all 16 registers simultaneously
Only the decoded register captures the data on the write clock edge
```
